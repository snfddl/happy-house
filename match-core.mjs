...
export function createMatcher(P, todayStr) {
  ...
  const fmtArea = ([a, b]) => { const x = Math.round(a), y = Math.round(b); return x === y ? `${x}` : `${x}~${y}`; };   // 정수㎡로 표시(사이트 면적표기와 통일), 맞는 평형 1개면 단일값(46~46→46)
  function areaMatch(req) {
    ...
    const hasHl = hl?.최소 != null;
    if (!areas.length && !hasHl) return { ok: null, range: null };   // 면적정보없음
    // 공고 전체범위(안맞음일 때 "공고에 뭐가 있는지" 표시용)
    let min = areas.length ? Math.min(...areas) : null, max = areas.length ? Math.max(...areas) : null;
    if (hasHl) { min = min == null ? hl.최소 : Math.min(min, hl.최소); max = max == null ? hl.최대 : Math.max(max, hl.최대); }
    // 전체범위 겹침이 아니라, 희망구간에 실제로 들어오는 개별 평형이 1개라도 있어야 맞음(한쪽만 입력해도 그 조건만 적용).
    const inRange = a => (wMin == null || a >= wMin) && (wMax == null || a <= wMax);
    const hits = areas.filter(inRange);
    if (hits.length) return { ok: true, range: [Math.min(...hits), Math.max(...hits)] };   // 맞음: 실제 맞는 평형 범위만 표시
    // 주택목록(매입형)은 개별 평형값이 없어 범위만 보유 → 희망구간과 겹치면 맞음(보수적 폴백)
    const hlOverlap = hasHl && !((wMin != null && hl.최대 < wMin) || (wMax != null && hl.최소 > wMax));
    if (hlOverlap) return { ok: true, range: [hl.최소, hl.최대] };
    return { ok: false, range: [min, max] };
  }
  ...
  function evaluateSale(req) {
    ...
    const fact = (label, v) => { if (확정(v)) 참고.push(`${label}: ${v.적용 === false ? '없음' : v.기간}`); };
    ...
    return {
      ...
      면적: area ? (area.ok == null ? '면적정보없음' : area.ok ? `맞음(${fmtArea(area.range)}㎡)` : `안맞음(공고 ${fmtArea(area.range)}㎡)`) : null,

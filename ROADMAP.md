...
- ✅ **조회 UX 2차**(2026-06-27): 카드 hover→해당 핀 강조(`onmouseenter/leave`+`selId` 추적, 터치엔 클릭 유지), `prefers-reduced-motion` 1블록(핀 펄스·hover 이동·transition 정지), 빈 상태 '필터 초기화' 버튼(`#emptyReset`→state 리셋+apply), **관심공고 찜**(별표 토글·`localStorage hh_fav_v1`·'⭐ 찜' 필터칩, 마감 알림 수익화 연결), **카드 본문 클릭=상세 모달**(hover=핀강조와 역할 분리, 이전 본문클릭=핀강조 cd9bf3f 되돌림), **마감임박 긴급강조**(오늘 마감·D-1→`.dday.urgent` 채운 빨강·깜빡임, D-2~3→연한 `.soon`). **남은 백로그**(메모리 `map-ux-todo`): a11y(공유공개 시점).

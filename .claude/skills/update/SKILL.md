...
### 2.6 단지형 공공임대 AI 참고분석 (비결정론, Sonnet 생성 + Opus 검증 — 선택)
접수중·접수예정 '단지형'(행복/국민/영구/통합공공/공공/50년) 임대만 대상(단일 단지라 시세비교 가능. 매입/전세는 호별 산재라 제외). 포커스=시세 대비 메리트+입지 트레이드오프+(분양전환형)전환 전망.
```bash
node build-lease-analysis-queue.mjs   # → data/lease-analysis-queue.json (단지별 prompt 포함)
```
- 큐가 비면 생략. 안 비면 분양과 동일 절차(Sonnet 생성 → Opus 적대검증). 결과는 `{ "<no>": {요약,확신도,출처,검증,검증노트,(source 선택)} }` 형태로 `data/lease-analysis-results.json`에 모은 뒤:
```bash
node inject-analysis.mjs --file=data/lease-analysis-results.json   # 소스무관(panId로 lh/sh/gh/myhome 탐색). source 명시 시 충돌가드. 멱등
```
- ⚠️ analysis-queue/results.json은 **gitignore된 일회성 staging**. 영구 기록은 requirements.json '참고분석'(커밋). 재생성 단계가 참고분석 보존해야 함(applyhome-derive 하드닝 완료, LH/SH는 추출=증분·normalize=in-place라 안전).

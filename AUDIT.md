...
- [x] **#11 index.json 무한누적**(485건 66% 마감) → **안전 prune 유틸 `prune-index.mjs`**(마감일 N일+ 경과건만 index-archive로 이관, 복구가능, 기본 dry-run·180일). **객관판단: 지금 공격적 prune은 해로움** — 마감 332건 중 328건이 90일 미만(소스 목록 잔존, 지우면 재다운로드). 180일 기준 현재 0건 = prune 불필요 입증. 메커니즘만 준비, --apply 미실행. · [x] **#12 gh-collect TODAY 재선언**(미사용 dead 선언이라 삭제) · **#13 pipeline/myhome-pipeline 스캐폴딩 중복**.

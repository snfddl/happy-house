...
메모리·`AUDIT.md`를 잠시 치우고 컨텍스트 0의 fresh 서브에이전트 3대(아키텍처·**정확도/매칭**·코드품질)로 재감사. Round 1이 놓친 이슈, 특히 **매칭 계산의 정답성** 문제가 다수 발견됨. **#1–#17 전건 처리 완료(2026-06-25). match-core 회귀테스트(`test-match-core.mjs` 17케이스) 신설.**
...
- [x] **#11 index.json 무한누적**(485건 66% 마감) → **안전 prune 유틸 `prune-index.mjs`**(마감일 N일+ 경과건만 index-archive로 이관, 복구가능, 기본 dry-run·180일). **객관판단: 지금 공격적 prune은 해로움** — 마감 332건 중 328건이 90일 미만(소스 목록 잔존, 지우면 재다운로드). 180일 기준 현재 0건 = prune 불필요 입증. 메커니즘만 준비, --apply 미실행. · [x] **#12 gh-collect TODAY 재선언**(미사용 dead 선언이라 삭제) · [x] **#13 pipeline/myhome-pipeline 스캐폴딩 중복** → 동일한 워커풀 `pool`을 collect-util로 추출(둘 다 import), myhome extractOne에 stderr 표면화(#4 일관성). extractOne/buildPrompt는 후처리(LH=fresh추출 vs 비LH=envelope병합)가 진짜 달라 각 파일 유지(근거 기록).
...
- [x] **#16** 루트 `dl_67253288`(342KB HWP, gitignore) 삭제 · `prep-slices.mjs` → `reference/`(경로 `../` 조정·README 등재) · [x] **#17** LH derived 5건 `상태:null` → index 상태(접수마감)·마감일 백필.

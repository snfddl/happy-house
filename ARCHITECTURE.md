...
- **오케스트레이션**: `process-all.mjs`(전 소스 진입점) · `pipeline.mjs`(LH 하위)
- **수집**: `lh-collect` · `applyhome-collect` · `myhome-collect` · `sh-collect` · `gh-collect` · `collect-util`(공통)
- **파생**: `slice-notice`(보일러플레이트 제거) · `applyhome-derive`(분양 결정론) · `myhome-pipeline`(myhome/sh/gh 추출) · `parse-housing-xlsx.py`(주택목록) · `inject-links`(원문링크)

...
- **파생**: `slice-notice`(보일러플레이트 제거) · `applyhome-derive`(분양 결정론·LLM 0, ⚠️기존 주입필드[참고분석·전매/실거주·PDF링크·건물유형] 이월 보존 — 단독 재실행 데이터유실 방지) · `myhome-pipeline`(myhome/sh/gh 슬라이스+추출) · `parse-housing-xlsx.py`(주택목록) · `inject-links`(원문링크)
...
- **AI 참고분석(비결정론·로컬·외부 API 0)**: `build-analysis-queue`(무순위/임의 분양) · `build-lease-analysis-queue`(단지형 임대) → Sonnet 생성 → Opus 적대검증 → `inject-analysis`(소스무관·panId 충돌가드). staging json은 gitignore, 영구기록은 requirements.참고분석
- **매칭**: `match-core`(로직 단일 소스 — 판정·가점·배점[미입력 floor='최소'표기]·임대 보조뷰 `rentView`[부담률·거주안정성]) · `match.mjs`(CLI) — 사이트는 인라인본(유형 대분류/소분류 `TYPE_GROUPS`·`groupOf`)

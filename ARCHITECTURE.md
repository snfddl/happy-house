...
- **빌드·운영**: `build-site`(→`site/index.html`, match-core·전 소스 derived·**Leaflet 벤더(`data/vendor/`) 인라인** + `geo-cache` 조인해 `좌표목록` 임베드) · `prune-index`(오래된 마감 아카이브) · `.github/workflows/refresh.yml`(CI). 사이트 UI에 **지도 연동**(핀↔카드·지역선택 줌·'이 지도 영역만' 가시영역 필터, 키 0·타일만 네트워크)

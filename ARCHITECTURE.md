...
- **`match-core.mjs`는 브라우저 인라인용 → 외부 import 금지** (`CLAUDE.md §4`, 필요한 헬퍼는 인라인 복제). CLI(`match.mjs`)는 Node라 match-core를 import해 공유하지만, match-core 자신은 `normalize-requirements`의 계층 캐논 함수를 import할 수 없어 양쪽에 복제된다 → `check-canon-drift.mjs`가 본문 동일성 assert로 드리프트를 차단(빌드 전, canonTier 함수 + TIER_ALIAS↔FIELD_SYN 금액 동의어 목록).
...
- **추출 골격**: `extract-core`(buildExtractPrompt mode=new/merge · runHeadless · postProcess · toQueueItem/mergeQueue → `extract-queue.json`, mergeQueue가 쓸 때마다 추출완료 잔존항목 자가정리) — pipeline·myhome-pipeline·`/update` 워크플로우(`update-extract.workflow.mjs`)가 공유하는 단일 소스
...
- **정규화·검증**: `normalize-requirements`(캐논화 + 단지형 임대 `건물유형`='아파트' + SH generic '임대'→공고명 기반 구체유형 보정) · `validate-requirements`(게이트 공통모듈) · `run-gate.mjs`(게이트 단독 실행 — 파이프라인 내장 게이트는 그 실행의 신규분만 보므로, 워크플로우 병렬 추출 후엔 이걸로. `--source=`/`--slugs=`, 전건 기본, 리포트 경로 동일) · `check-canon-drift`(드리프트 가드)
...
- **빌드·운영**: `build-site`(→`site/index.html`, 게이트 fail(필수필드·enum·JSON깨짐)건은 인라인 제외·경고, match-core·전 소스 derived·**Leaflet 벤더(`data/vendor/`) 인라인** + `geo-cache` 조인해 `좌표목록` 임베드. 인라인 직전 **비렌더 내부/감수 필드 blocklist strip**(`_검증노트`·`선정방식상세`·`첨부.name` 등 매처·템플릿 미사용분, ~27%↓; source는 보존, `원문링크.로컬PDF` 절대경로 누출도 차단)) · `prune-index`(오래된 마감 아카이브) · `.github/workflows/refresh.yml`(CI). 사이트 UI에 **지도 연동**(핀↔카드·지역선택 줌·'이 지도 영역만' 가시영역 필터, 키 0·타일만 네트워크)
- **알림(구독·발송, 로컬 전용·외부 LLM 0)**: `notify.mjs`(process-all 마지막 단계) — 활성 공고를 구독자 프로필로 `match-core` 매칭해 `sent`에 없는 매칭분만 Resend로 이메일 다이제스트 발송(매칭=결정론·저장/조회=Supabase REST·발송=Resend REST, 전역 fetch·신규 의존성 0). env(SUPABASE_URL/SERVICE_KEY·RESEND_API_KEY) 없으면 graceful skip(CI·타인 환경 무결). 중복발송 방지의 단일 권위=`sent` 테이블, 구독자당 하루 상한=`NOTIFY_DAILY_CAP`. 견고성: REST 호출 타임아웃 30s, `sent` 이력 조회 실패 시 fail-closed 중단(빈 이력 진행=전량 재발송 방지), 구독자 단위 try/catch 격리(불량 프로필 1명이 배치를 못 죽임), sent 기록/카운터 갱신 실패는 경고 표면화. 저장소=Supabase Postgres `supabase/schema.sql`(`subscribers`·`sent`·RLS·`unsubscribe` RPC): anon(브라우저)은 **가입 INSERT·해지 RPC만**, PII 읽기/수정은 RLS로 차단, service_role(notify)만 전체 접근. 가입 UI=`site/_template.html` '🔔 알림 받기'(build-site가 `SUPABASE_URL`+`SUPABASE_ANON_KEY` env 주입 시에만 노출, anon키는 공개 의도), 해지=`site/_unsubscribe.html`→`unsubscribe.html`(토큰=`subscribers.id`로 `unsubscribe` RPC, 서버 0)
